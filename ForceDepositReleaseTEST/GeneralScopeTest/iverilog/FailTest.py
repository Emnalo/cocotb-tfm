...
async def FailTest(dut):
    ...
    async def inject_and_check(signal, fault_value, expected_value, description, method='force'):
...
#    await inject_and_check(dut.array_signal, 0b0000, 0b0001, "Fault injection in array_signal") #Error type
    await inject_and_check(dut.array_constant, 0b1000,0b1111, "Fault injection in array_constant")
    await inject_and_check(dut.U_Inversor.entrada, 0b1000,0b0001, "Fault injection in entrada inversor")
    await inject_and_check(dut.U_Inversor.salida, 0b1000,0b1110, "Fault injection in salida inversor")
    await inject_and_check(dut.counter, 1, 1, "Fault injection in counter")
